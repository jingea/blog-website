...
首先是class文件格式验证, 保证输入的字节流能正确地解析并存储于方法区之内.确保符合Class文件规范,且能被当前版本的虚拟机处理.
...
### 准备
准备阶段是正式为类变量分配内存并设置类变量初始值的阶段,这些内存都将在方法区中进行分配. 这个阶段中有俩个容易产生混淆的概念需要强调一下,首先是这时候进行内存分配的仅包括类变量,而不包括实例变量,实例变量将会在对象实例化时随着对象一起分配在java堆中. 其中是这里所说的初始值"通常情况"下是数据类型为0.例如:
```java
...
类或接口解析对`CONSTANT_Class_info`结构体进行解析
...
字段解析对`CONSTANT_Fieldref_info`结构体进行解析
...
类方法解析对`CONSTANT_Methodref_info`结构体进行解析
...
接口方法解析对`CONSTANT_InterfaceMethodref_info`结构体进行解析
